...
import { g as getTransformedPath, p as pascalCase, s as stringifyComponentImport, D as DIRECTIVE_IMPORT_PREFIX, a as DISABLE_COMMENT, b as parseId, m as matchGlobs, c as getNameFromFilePath, i as isExclude, n as normalizeComponetInfo, r as resolveAlias, d as shouldTransform } from './shared/vite-plugin-uni-components.8d653dd6.mjs';
export { e as camelCase, k as kebabCase } from './shared/vite-plugin-uni-components.8d653dd6.mjs';
import { resolve, join, relative as relative$1 } from 'node:path';
import Debug from 'debug';
import { toArray, slash, notNullish, throttle } from '@antfu/utils';
import { isPackageExists, getPackageInfoSync, importModule } from 'local-pkg';
import fg from 'fast-glob';
import { dirname, isAbsolute, relative } from 'path';
import { existsSync } from 'fs';
import { readFile, mkdir, writeFile as writeFile$1 } from 'fs/promises';
...
const debug$4 = Debug("vite-plugin-uni-components:glob");
...
const debug$3 = Debug("vite-plugin-uni-components:transform:component");
...
const debug$2 = Debug("vite-plugin-uni-components:transform:directive");
...
const debug$1 = Debug("vite-plugin-uni-components:transformer");
...
class Context {
  ...
  onUpdate(path) {
    ...
    Object.entries(this._componentUsageMap).forEach(([key, values]) => {
      if (values.has(name)) {
        const r = `/${slash(relative$1(this.root, key))}`;
  ...
  relative(path) {
    ...
    return slash(relative$1(this.root, path));
  }
  /**
...
function VitePluginComponents(options = {}) {
  ...
  return {
    ...
    configResolved(config) {
      ...
      if (ctx.options.dts) {
        ctx.searchGlob();
        if (!existsSync$1(ctx.options.dts))
...
export { VitePluginComponents as default, pascalCase };
